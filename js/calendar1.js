...
(function($, window, document, undefined) {
    ...
    Calendar.prototype = {
        ...
        showCalendar: function() {
            ...
            //请求数据
            $.ajax({
                url: "http://223.6.252.143:30005/spmag/callSp",
                type: "POST",
                data: {
                    name: 'SP_Month_Reminder',
                    params: '1036319921,542279989560',
                    // params: '601879343,43953268044'
                },
                dataType: "json",
                async: false,
                success: function(data) {
                    // console.log(data);
                    self.$calendarTitle_text.text(year + '/' + dateStr.substr(4, 2));
                    self.$calendarDate_item.each(function(i) {
                        ...
                        /*将请求过来的数据日期和当前日期比较，如果日期相等，则变红 */
                        for (var i = 0; i < data.length; i++) {
                            if (Number(allDay_str) == data[i]['日期']) {
        ...
        renderDOM: function() {
            ...
            var _titleStr = '<a href="#" class="_title"></a>' +
        ...
        inital: function() {
            ...
            this.$calendarTitle_text = this.$calendar_title.find('._title');
    ...
    var dateObj = (function() {
        var _date_ = new Date().getTime() - (24 * 60 * 60 * 1000);
        var _date = new Date(_date_);
        return {
            getDate: function() {
                return _date;
            },
